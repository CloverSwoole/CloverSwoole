...
class Command extends \CloverSwoole\CloverSwoole\Facade\Command\Abstracts\Command
{
    ...
    protected function start()
    {
         ...
         * 获取server
         */
        $http = new \Swoole\Http\Server($this -> config['host'], $this -> config['port']);
        /**
         * 判断参数是否存在
         */
    ...
    protected function stop()
    {
         ...
         */
        if(isset($this -> options['pid']) && file_exists($this -> options['pid'])){
            Framework::getContainerInterface()['config']['swoole_http']['server']['pid_file'] = $this -> options['pid'];
        }
        if((!isset(Framework::getContainerInterface()['config']['swoole_http']['server']['pid_file'])) || (!file_exists(Framework::getContainerInterface()['config']['swoole_http']['server']['pid_file']))){
            echo "pid files no exists\n";
            return ;
        }
        $pid = file_get_contents(Framework::getContainerInterface()['config']['swoole_http']['server']['pid_file']);
